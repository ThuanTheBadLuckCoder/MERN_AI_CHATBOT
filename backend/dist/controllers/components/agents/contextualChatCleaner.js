...
export class ContextualChatCleaner {
     ...
     */
    static cleanChatHistoryForCodeContext(conversation, currentInput, options = {}) {
        const { maxMessages = 15, prioritizeCodeEvolution = true, includeUserRequests = true, includeCodeExplanations = true } = options;
        ...
        let relevantMessages = cleanedMessages.filter(msg => {
            ...
            if (prioritizeCodeEvolution && msg.isCodeRelated)
                return true;
            // Include user requests if enabled
            if (includeUserRequests && msg.isUserRequest)
                return true;
            // Include messages with code blocks if including explanations
            if (includeCodeExplanations && msg.hasCodeBlock)
                return true;
        ...
        relevantMessages.sort((a, b) => cleanedMessages.indexOf(a) - cleanedMessages.indexOf(b));
    ...
    static isCodeRelated(content) {
        ...
        return codeIndicators.some(indicator => content.toLowerCase().includes(indicator.toLowerCase()));
    }
    /**
     * Check if message is a user request for code changes/creation
     */
    static isUserRequest(content) {
     ...
     */
    static calculateMessageImportance(message, currentInput) {
     ...
     */
    static isRelatedToCurrentInput(messageContent, currentInput) {
     ...
     */
    static toLangChainMessage(message) {
     ...
     */
    static getCodeEvolutionContext(conversation, maxSteps = 5) {
     ...
     */
    static getCurrentRequirementsContext(conversation, currentInput) {
        if (!conversation?.messages)
            return '';
...
export const getContextualChatHistory = (conversation, currentInput, maxMessages = 15) => {
    return ContextualChatCleaner.cleanChatHistoryForCodeContext(conversation, currentInput, {
        maxMessages,
...
export const getEnhancedCodeContext = (conversation, currentInput) => {
...
//# sourceMappingURL=contextualChatCleaner.js.map