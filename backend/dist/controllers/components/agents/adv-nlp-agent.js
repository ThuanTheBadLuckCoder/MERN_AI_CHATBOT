...
const intentAnalyzerTool = new DynamicTool({
    ...
    func: async (input) => {
        try {
            ...
            const hasCodeContext = codeContext && (codeContext.fullHtmlDocument ||
...
export async function processWithAdvancedNLP(input, chatHistory = [], codeContext = null, conversationId = "default", executeCallback) {
    ...
    try {
        ...
        if (intentAnalysis.needsClarification || intentAnalysis.ambiguityScore > 0.6) {
            ...
            if (disambiguation.needsDisambiguation) {
                ...
                if (disambiguation.options && disambiguation.options.length > 0) {
                    clarifyingResponse += "\n\nOptions:";
                    disambiguation.options.forEach((option, index) => {
                        clarifyingResponse += `\n${index + 1}. ${option}`;
                    });
                }
        ...
        const executorResult = await executeCallback(enhancedQuery.enhancedQuery, chatHistory, conversationId);
...
export async function handleDisambiguation(originalQuery, clarificationResponse, intentAnalysis, chatHistory, codeContext = null, conversationId = "default", executeCallback) {
...
export function isDisambiguationResponse(input, chatHistory, conversationId = "default") {
...
export async function processDisambiguationResponse(input, chatHistory, codeContext = null, conversationId = "default", executeCallback) {
    const disambiguationState = global.disambiguationState[conversationId];
    ...
    return executeCallback(enhancedQuery.enhancedQuery, chatHistory, conversationId);
}
// Export all the tools and functions
export { intentAnalyzerTool, disambiguationTool, queryEnhancementTool };
//# sourceMappingURL=adv-nlp-agent.js.map