...
function prioritizeAndDeduplicate(documents) {
    ...
    childDocuments.forEach(doc => {
        if (doc.metadata?.parent_id) {
            const parentExists = result.some(resultDoc => resultDoc.metadata?.document_id === doc.metadata?.parent_id);
            if (!parentExists) {
                const docId = doc.metadata?.document_id || doc.pageContent.slice(0, 100);
                if (uniqueDocuments.has(docId)) {
                    result.push(doc);
...
function detectStructuralDeviations(response, context) {
    const deviations = [];
...
function extractHTMLStructure(text) {
    return text
        .replace(/\s+/g, ' ')
        .replace(/<[^>]*>/g, (match) => {
