...
import { AgentExecutor, type AgentStep, type AgentAction, type AgentFinish } from "langchain/agents";
...
const mongoDBSimilarTool = new DynamicTool({
  ...
  func: async (input) => {
    try {
      ...
      const similar = await QuestionAnswer.find(
        { $text: { $search: input } },
      ...
      if (similar && similar.length > 0) {
        const results = similar.map(qa => 
          `SIMILAR QUESTION: "${qa.question}"\nANSWER: ${qa.answer}`
        ).join("\n\n");
        return results;
      } else {
