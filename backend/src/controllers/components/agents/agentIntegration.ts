    ...
    func: async (input: string) => {
        try {
            ...
            const unauthorizedImages = responseImages.filter(img => {
                ...
                if (img.startsWith('<svg')) return false;
 ...
 */
function isPlaceholderOrVirtualImage(reference: string): boolean {
    // Skip SVG content for this check
    if (reference.startsWith('<svg')) return false;
...
function normalizeImageUrl(url: string): string {
    if (url.startsWith('<svg')) return url; // Skip SVG content
 ...
 */
const verifyAndCorrectImages = async (
...
export {
    searchResultImageVerificationTool,
    enhanceHybridSearchTool,
    imagePreservationPostProcessor,
    verifyAndCorrectImages,
    createImagePreservingExecuteWithNLP,
